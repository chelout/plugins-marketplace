 drawKind.flow=function(rr){var T=tracks();
  ...
  E.forEach(function(e){var g=document.createElementNS(NS,'g');g.setAttribute('data-e',e.a+' '+e.b);
   var pts=e.path.map(function(p){return{x:bx(p[0])+p[2],y:base(p[1])+p[3]}});
   ...
   /* label offsets (6, 3, row e.ly from base()) are mirrored by the room check in diagrams/flow.py, which measures lines on a row by their offset from that base */
   if(e.label){var t=document.createElementNS(NS,'text'),a0=pts[0],lx,ly,anc='start';
    if(pts.length>=3){var p1=pts[1],p2=pts[2],oy=e.path[1][3]||0;if(p2.y===p1.y){var rt=p2.x>p1.x;lx=rt?p2.x-6:p2.x+6;ly=p1.y-oy-9;anc=rt?'end':'start'}else{var my=(e.ly!=null?base(e.ly):(p1.y+p2.y)/2)+4;if(e.ls==='L'){lx=p1.x-6;ly=my;anc='end'}else{lx=p1.x+6;ly=my}}}else if(e.sa==='B'){lx=a0.x+5;ly=a0.y+14}else if(e.sa==='T'){lx=a0.x+5;ly=a0.y-6}else if(e.sa==='R'){lx=a0.x+3;ly=a0.y-5}else{lx=a0.x-3;ly=a0.y-5;anc='end'}
